class dsAlert {
    ...
    // Shortcut methods for different alert types inside class
...
(function (root, $, undefined) {
  ...
  toastLib.showToast = function (options) {
	...
	// If options has `msg`, assign it to `message` 
	if (options && options.msg) { settings.message = options.msg; }
...
//toastalert.showToast({message:'No Session. Redirecting to Login', status:'error'});

function goMsg(res, array ={}) {
    if (res.status === 'data') {
        console.log("Response in goMsg", res);
    } else if (res.status === 'html') {
        //console.log("Arr in goMsg", array);
        //console.log("Response in goMsg", res);
        if (array && array.div) {
            document.getElementById(array.div).innerHTML = res.content.html;
        } else {
            console.warn("No target div provided in array");
        }
    } else {
        window.toastalert.showToast(res);
    }
}
...
function btnsLoading(action) {
  ...
  if (loadingElements) {
    for (let element of loadingElements) {
      element.style.display = action ? "block" : "none"; // Show if action is true, hide otherwise
    }
  }
}
...
$(document).on("click", "button[type='submit'], input[type='submit']", async function (e) {
    ...
    if (form.data("ajax") !== true) {    return;   }// allow PHP POST + page reload

    e.preventDefault();
    let formID = form.attr("id"); // get form id
    if (!formID) {      console.warn("No form ID found — stopping submit");      return;    }
    // Get form data
...
//    console.log("Form Data:", [...formData]);
    ...
    ajaxSubmit(arr, path = actionPath)
    .then((response) => {
        const callbackName = form.data("callback");
        if (callbackName && typeof window[callbackName] === "function") {
            window[callbackName](response);   // call function by name
        } else {	goMsg(response, arr);	}
    })
    .catch((error) => {
        console.log(error.message);
        goMsg({status: "error", message: "Opps!!! Something went wrong: "});
    });
});

function myPost(arr, path = '') {
    let finalPath = path === '' ? actionPath : path;
    ajaxSubmit(arr, finalPath)
    .then((response) => {
        if (typeof arr !== "undefined" && arr.callback) {
            const callbackName = arr.callback;
            if (callbackName && typeof window[callbackName] === "function") {
...
function ajaxSubmit(arr, path) {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            timeout: 0,
            beforeSend: function () {     btnsLoading(false);       },
