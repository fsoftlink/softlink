...
function ajaxSubmit(arr, path) {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            timeout: 0,
            beforeSend: () => btnsLoading(false),
            success: (res) => resolve(res),
            error: (xhr, status, error) => {
                reject({ status: "error", msg: "AJAX Error", detail: error });
            },
            complete: () => btnsLoading(true)
        });
    });
}
...
function goMsg(res, array = {}) {
    ...
    else if (res.status === 'data') {      console.log("Response status data :", res);  } 
    else {     toastalert.showToast(res);    }
}
